...
describe('equal-area warped radial sectors', () => {
  ...
  it('keeps a clamped visual node mapped to its own sector', () => {
    ...
    const middleSector = layout.sectors[1];

    expect(findEqualAreaSector(layout, middleSector.labelPoint)?.id).toBe(
      middleSector.id,
    );
  });
