...
export function TransactionFlow() {
  ...
  const { lastError } = useTransactions();
  ...
  useEffect(() => {
    ...
    if (lastError === lastErrorRef.current) {
      return;
    }
    lastErrorRef.current = lastError;
    handleToast(lastError);
  }, [handleToast, lastError]);
  ...
  const handleEditTransaction = useCallback(
    (t: TransactionRecord) => {
      // Transaction data comes directly from Google Sheets with sheetRow
  ...
  const handleDelete = useCallback(() => {
    ...
    deleteMutation.mutate(
      { id: editingTransaction.id, sheetRow: editingTransaction.sheetRow },
      {
        onSuccess: () => {
          resetFlow();
        },
