...
export function TopDashboard({ onEditTransaction }: TopDashboardProps) {
  ...
  const [visibleDate, setVisibleDate] = useState<string>(() => format(new Date(), 'yyyy-MM-dd'));
  ...
  const headerOpacity = useTransform(smoothScrollY, [-30, 0, 30], [0.7, 1, 0.7]);
  ...
  const totalsByDate = useMemo(() => {
    ...
    for (const [dateKey, txns] of Object.entries(transactionsByDate)) {
      ...
      for (const t of txns) {
        if (t.type === 'expense') {
          totals[dateKey][t.currency] = (totals[dateKey][t.currency] || 0) + Number(t.amount);
        }
      }
    }
    return totals;
          ...
          {isLoading ? (
            ...
            <div className="space-y-0.5">
              {displayList.map((t, idx) => {
                ...
                const prevDate =
                  idx > 0 ? format(parseDate(displayList[idx - 1].date), 'yyyy-MM-dd') : null;
                ...
                return (
                  <div key={t.id}>
                    ...
                    >
                      ...
                      >
                        {t.type === 'expense' ? '' : '+'}
                        {t.currency === 'THB' ? '฿' : t.currency === 'USD' ? '$' : t.currency}
